...
const CLAUDE_MD_ROOT: FileTreeItem = {
  path: "CLAUDE.md",
  color: "#7c6ff7",
  target: "전역 철학",
...
const CLAUDE_MD_CHILDREN: FileTreeItem[] = [
  {
    ...
    >
      ...
      <DetailSection id="s-context">
        ...
        <Lead>
          ...
          바이브코딩처럼 8일 안에 QA·기획·개발을 다 끝내야 하는 환경에서는 사람의 생각이 곧 병목이라,
          사람 중심 워크플로우(예쁜 코드, 정교한 아키텍처)를 그대로 들고 가면 안 된다.
          AI가 막힘 없이 작업할 수 있도록 '필요한 정보를 그때그때 정확한 위치에 적치하는 워크플로우' 자체를 1순위로 설계했다.
        </Lead>
        <Lead>
          1명+AI는 한 사람이 자기 컨텍스트를 책임지면 됐는데, 4명+같은 AI는 다르다.
          디렉토리 설계까지 AI에 맡기다 보니 잘못된 위치에 코드가 한 번 자리 잡으면 거기서 이상한 코드가 계속 자라나는
          <strong className="text-foreground"> 공유지의 비극</strong>이 발생했다 —
          같은 도구를 쓴다는 사실이 같은 컨텍스트 공유를 의미하지 않는다.
        </Lead>
          ...
          steps={[
            {
              ...
              metas: [
                { key: "현상", body: <>큐 알고리즘에 변수명을 스택으로 주면 AI가 변수명에 끌려가 응답 품질이 떨어졌다 — var_mapping 정의가 어긋날 때마다 누가 수정했는지 잊는 패턴이 반복됐다.</> },
              ],
            },
            {
              num: "Step 2",
              eyebrow: "딜레마",
              title: "AI용 문서와 사람용 문서, 둘 다 소스 변경에 자동으로 따라오지 않는다",
              metas: [
                { key: "충돌", body: <>자동화로 따라오게 하면 검증이 무거워져 토큰을 잡아먹고, 그 토큰 비용이 시각화 품질을 깎는다.</> },
              ],
            },
            {
              num: "Step 3",
              eyebrow: "패턴 환원",
              title: "4개 리스크 = '컨텍스트 비대칭과 토큰 예산' 자원 충돌",
              isFinal: true,
              metas: [
                { key: "이어짐", body: <>"같은 사이클의 다른 시점"으로 환원되며, 컨텍스트와 토큰을 어떻게 적치하느냐가 다음 AI 협업 파이프라인의 1번 설계 기준이 됐다.</> },
              ],
            },
          ]}
        />
        ...
        <BulletList
          items={[
            <><strong className="text-foreground">조용한 계약 위반:</strong> AI 응답 필드 하나가 프롬프트 수정으로 빠지면 타입 캐스팅은 통과하고, 해당 필드 참조 패널에서만 늦게 터진다.</>,
            <><strong className="text-foreground">트레이스 병합 무결성:</strong> <Inline>rawTrace</Inline>·<Inline>annotated</Inline> 길이가 다를 때 수동 병합을 허용하면 스텝 인덱스가 어긋나 잘못된 줄에 해설이 붙는다.</>,
            <><strong className="text-foreground">반복 검증 비용:</strong> PR마다 lint · typecheck · build · 계약을 사람이 순서대로 돌리면 대회 속도와 상충한다.</>,
            <><strong className="text-foreground">팀 컨텍스트 공유:</strong> 같은 Claude를 쓰는 4명에게 "이 레이어 금지"를 매번 말로 설명할 수 없다 — 구조로 강제해야 한다.</>,
          ]}
        />

        <Callout label="핵심 과제">
          <strong>훅으로 강제할 것과 커맨드로 유도할 것의 경계를 어떻게 설계할까?</strong>
          {" "}"까먹을 수 없는 자동화"와 "의도적으로 호출하는 검증"의 분리가 출발점이었다.
        </Callout>
      </DetailSection>
            ...
            <SplitLayout
              ...
              excluded={
                <>
                  ...
                  <DiscoveryBlock>
                    초기엔 루트에 코드 컨벤션만 두고 시작했지만, 4명이 동시 개발하면서 Claude가 작업과 무관한 영역까지 분석하느라 토큰을 잡아먹었다.
                    그렇다고 루트에 아키텍처를 넣자니 무관한 feature까지 매번 로드되는 문제가 생겨, 결국 '레이어 디렉토리에 그 레이어 규칙만'이라는 계층형 분리로 전환했다.
                  </DiscoveryBlock>
                </>
              }
            />
            ...
            <SplitLayout
              ...
              excluded={
                <>
                  ...
                  <DiscoveryBlock>
                    응답 스키마와 어긋나는 프롬프트 수정이 들어오면 전혀 관련 없어 보이는 영역에서 사고가 터지는데, Zod 런타임만으로는 터진 뒤에야 catch된다.
                    그래서 변경 유형별로 단계별 차단 — <Inline>/prompt-diff</Inline> · <Inline>/trace-validate</Inline> · <Inline>/pr-ready</Inline> — 구조로 갔다.
                  </DiscoveryBlock>
                </>
              }
            />
      ...
      <SectionGap>
        <DetailSection id="s-solution">
          ...
          <FileTree rootItem={CLAUDE_MD_ROOT} items={CLAUDE_MD_CHILDREN} />
      ...
      <SectionGap>
        <DetailSection id="s-result">
          ...
          <Lead>
            ...
            <strong className="text-foreground"> 설계 변경 순간에 계약 문서가 코드와 함께 움직이는가</strong>가 전체 품질을 결정했다 —
            다음 AI 협업 파이프라인의 1번 설계 기준이 여기서 나왔다.
          </Lead>

          <CategoryBlock num="01" name="팀 변화" sub="질문이 사라진 흔적">
            <CategorySingle
              title="다른 팀원의 파트 설계 흐름을 별도 질문 없이 따라갈 수 있게 됐다"
              body={<>레이어별 CLAUDE.md에 아키텍처가 박혀 있어 컨텍스트만 보면 흐름이 잡혔다. 작업 범위에 맞는 컨텍스트만 로드되니 Claude plan 모드 응답성도 함께 개선됐다.</>}
            />
          </CategoryBlock>
          ...
          <CategoryBlock num="02" name="협업 책임 이동" sub="/pr-ready 체인 도입 후">
            <CategorySingle
              ...
              body={<>PR 작성자가 머지 전에 자가 검증을 끝내게 되면서, 자기 작업 컨텍스트가 살아있는 상태에서 수정하니 버그 발생 자체가 확연히 줄었다.</>}
            />
          </CategoryBlock>
          ...
          <CategoryBlock num="03" name="의사결정 비용" sub="채택 2 · 미채택 3 같은 문서">
            <CategorySingle
              ...
              body={<>Feature별 의사결정 로그(<Inline>prompts/optimization-cash-token/</Inline>)에 채택·미채택이 같이 박제돼 있어 같은 최적화 시도를 재검토하는 비용이 사라졌다.</>}
            />
          </CategoryBlock>

          <CategoryBlock num="04" name="메타 인식" sub="공유지의 비극과 자원 충돌 환원">
            <CategorySingle
              title="4개 리스크가 결국 '컨텍스트 비대칭과 토큰 예산' 한 자원 충돌로 환원됐다"
              body={<>같은 도구를 쓴다는 사실이 같은 컨텍스트 공유를 의미하지 않는다 — 이 회고가 다음 AI 협업 파이프라인의 1번 설계 기준이 됐다.</>}
            />
          </CategoryBlock>
